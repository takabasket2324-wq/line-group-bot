...
// ---------------------------------------------------------------------------
// 相談ハンドオフ（「相談したい」→連絡先受領を診断リードに記録）
// 列K〜M：相談状況 ／ 連絡先 ／ 相談日時
// ---------------------------------------------------------------------------

const CONSULT_HEADER = ["相談状況", "連絡先", "相談日時"];

let _consultHeaderReady = false;

/** 診断リードシートのK1:M1に相談用ヘッダーが無ければ追記 */
async function ensureConsultColumns() {
  if (_consultHeaderReady) return;
  await ensureLeadsSheet();

  const sheets = await getSheets();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `'${LEADS_SHEET}'!K1:M1`,
  });
  const row = res.data.values?.[0] || [];
  if (row[0] !== CONSULT_HEADER[0]) {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `'${LEADS_SHEET}'!K1:M1`,
      valueInputOption: "RAW",
      requestBody: { values: [CONSULT_HEADER] },
    });
    console.log(`[sheets] 診断リードに相談列(K〜M)のヘッダーを追加`);
  }
  _consultHeaderReady = true;
}

/**
 * 相談希望（連絡先受領）を診断リードに記録する。
 * - 同じuserIdの行があれば「最新の行」のK〜Mを更新
 * - 無ければ（診断未経験ユーザー）新しい行を作る
 * @param {{ userId, displayName, contactText }} p
 * @returns {{ storeName, total, existing }} 通知文用（診断履歴の店名・総合%）
 */
export async function recordConsultContact({ userId, displayName, contactText }) {
  await ensureConsultColumns();

  const sheets = await getSheets();
  const now = new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" });

  // userId（C列）が一致する最新行を探す
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `'${LEADS_SHEET}'!A2:J`,
  });
  const rows = res.data.values || [];
  let rowIndex = -1;
  for (let i = rows.length - 1; i >= 0; i--) {
    if ((rows[i][2] || "") === userId) { rowIndex = i; break; }
  }

  if (rowIndex >= 0) {
    const sheetRow = rowIndex + 2; // A2起点 → 実シート行番号
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `'${LEADS_SHEET}'!K${sheetRow}:M${sheetRow}`,
      valueInputOption: "RAW",
      requestBody: { values: [["相談希望🔥", contactText, now]] },
    });
    return {
      storeName: rows[rowIndex][3] || "",
      total: rows[rowIndex][4] || "",
      existing: true,
    };
  }

  // 診断未経験ユーザー → 新規行（診断系の列は空のまま）
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    ...
    requestBody: {
      values: [[
        now, displayName ?? "", userId ?? "",
        "", "", "", "", "", "", "",
        "相談希望🔥", contactText ?? "", now,
      ]],
    },
  });
  return { storeName: "", total: "", existing: false };
}
